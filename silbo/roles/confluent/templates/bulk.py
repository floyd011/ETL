...
def write_database(retval, sql_dict, start_time, table, partition):
    ...
    logging.info("%s - partition %s, Last Commited offset: %d, Last tx time: %s, Duration batch: %f, Counted: %d" % (table, partition.id, retval[5], retval[4], duration, retval[6] ))
    return 0
...
class readThread (threading.Thread):
    ...
    def __init__(self, threadID, c, sdict, topic, part):
        ...
        self.topic = topic
        self.part = part
        self.consumer = self.topic.get_simple_consumer(consumer_group=CONSUMER_GROUP,partitions=(self.part,), consumer_timeout_ms=5000, auto_commit_enable=False,use_rdkafka = False)
         
            
    def run(self):
        try:    
            logging.warning('Started %s Topic: %s Partition %s' % (self.getName(), self.sdict, self.part.id))
            global PAUSED
            self.consumer.start()
                    ...
                    if self.sdict not in PAUSED:
                        ...
                        if duration > SAVETIME or retval[3]:
                            if len(retval[1]) > 0 or len(retval[2]) > 0:
                                ret = write_database(retval, sql_dict, start_time, self.sdict, self.part)
                                if ret != 0:
                                    PAUSED += self.sdict
            ...
            logging.warning('Error in Thread %s Topic: %s Partition: %s, error: %s' % (self.getName(), self.sdict, self.part, e))
        finally:
            self.consumer.stop()
            logging.warning('Stopped %s Topic: %s Partition: %s' % (self.getName(), self.sdict, self.part.id))
            ...
            for message in self.consumer:
                ...
                if message is not None:
                    ...
                    if message.value is not None:
                        ...
                        msg_val['kafka_part'] = self.part.id
        ...
        for i in topic_set:
            if i not in exclude_set and i in staging_set:
                ...
                for p in topic.partitions:
                    readt = readThread(readthreadID, c, i, topic, parts[p])
                    readt.start()
                    reads.append(readt)
                    readthreadID += 1
