...
def db_section(sdict, topic):
    try:
        ...
        if INITIAL == "Y":
            consumer = topic.get_simple_consumer(consumer_group=CONSUMER_GROUP,consumer_timeout_ms=5000, auto_commit_enable=False,use_rdkafka = True, auto_offset_reset=OffsetType.EARLIEST, reset_offset_on_start=True)
        else:
            consumer = topic.get_simple_consumer(consumer_group=CONSUMER_GROUP,consumer_timeout_ms=5000, auto_commit_enable=False,use_rdkafka = True)
        #consumer.start()
        parts = topic.partitions[0]
        listcons.append(tuple((consumer, sdict, parts)))
...
def ReadConsumer(c, consumer, sdict, part):
    ...
    try:
        ...
        if PAUSED != '*':     
            if sdict not in PAUSED:
                ...
                retval = export(consumer, part, retval, start_time)
                duration = time.time() - start_time
...
def export(consumer, part, retval, start_time):
...
def main():        
    ...
    try:
        ...
        while True:
            time.sleep(0.5)
            [ReadConsumer(c, x[0], x[1], x[2]) for x in listcons]
            if terminate:                                           
                break 
