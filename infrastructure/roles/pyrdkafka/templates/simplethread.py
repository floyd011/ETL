...
class readThread (threading.Thread):
    ...
    def __init__(self, threadID, c, sdict, topic, part):
        ...
        self.topic = topic            
        self.part = part
        self.consumer = topic.get_simple_consumer(consumer_group=CONSUMER_GROUP,consumer_timeout_ms=5000, auto_commit_enable=False,use_rdkafka = True)
    ...
    def export(self, retval, start_time):
        ...
        try:
            for message in self.consumer:
                ...
                if message is not None:
                    ...
                    if message.value is not None:
                        ...
                        count += 1
                        offset_list.append(tuple((self.part, message.offset)))
            ...
            llogging.warning('Error in Thread %s Topic: %s , error: %s' % (self.getName(), self.sdict, e))
        except e:
    ...
    try:
        ...
        for i in topic_set:
            if i not in exclude_set and i not in staging_set:
                ...
                time.sleep(1)
                db_section(i)
                readt = readThread(readthreadID, c, i, topic, topic.partitions[0])
                readt.start()
                reads.append(readt)
                readthreadID += 1
